...
export const SwiperImages = ({ images }: SwiperImagesProps) => {
  return (
    <Swiper
      ...
      className="mySwiper bg-background-foreground/85 rounded-lg w-full md:w-2/3"
    >
      {images.map((image, index) => (
        <SwiperSlide key={index} className="flex justify-center items-center">
          <div className="flex justify-center items-center relative overflow-hidden rounded-lg aspect-video w-full">
            <Image
              src={image}
              alt={`Image ${index}`}
